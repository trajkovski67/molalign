...
def run_xtb(xyz_file, charge=0, solvent=None):
    """Run xTB OHess calculation and extract total free energy"""
    base = os.path.splitext(xyz_file)[0]
    cmd = ["xtb", xyz_file, "--ohess", "--chrg", str(charge)]
    if solvent:
        cmd += ["--alpb", solvent]
...
def main():
    parser = argparse.ArgumentParser(description="Compute ΔGsolv and ΔGgas using xTB --ohess for top 5 complexes")
