#!/usr/bin/env python3
"""
driver_orca_parallel.py

molalign-extended-parallel (ORCA version)
...
Pipeline:
  ...
  4. For each scale:
        - Run all_grid_orca_parallel.py (which scans all grids in parallel)
        - JSON output is identical in structure to the original TB-lite code.

New:
  - Uses ORCA via OPI (r2SCAN-3c + CPCM) as the backend (in the grid script).
  - Keeps the same external tools: moist, cpcm-reader
"""

import os

# -------------------------------------------------------------------
# FORCE all heavy libraries to single-thread BEFORE importing numpy/etc
# -------------------------------------------------------------------
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Solute XYZ + moist (scaled vdW) + solvent CPCM + ORCA/OPI alignment."
    )
    ...
    p.add_argument("--charge", type=int, default=0,
                   help="Total charge for r2SCAN-3c+CPCM calculation (default: 0)")
    ...
    p.add_argument("--ncores", type=int, default=None,
                   help="Max number of Python workers for all_grid_orca_parallel "
                        "(sets ALIGN_TB_MAX_WORKERS). If omitted, the driver "
...
def main():
    ...
    # Propagate core limit to ORCA parallel driver
    if args.ncores is not None and args.ncores > 0:
        os.environ["ALIGN_TB_MAX_WORKERS"] = str(args.ncores)

    print("\n========== MOIST + MOLALIGN + ORCA r2SCAN-3c + CPCM (SCALED vdW RADII) ==========")
    ...
    if args.ncores is not None:
        ...
        print("Max workers  : (driver default, typically 4)")
    print("------------------------------------------------------------------")
    ...
    for scale in tqdm(args.scales, desc="Scales", unit="scale"):
        ...
        out_json = os.path.join(outdir, f"orca_r2scan3c_cpcm_results_scale{tag}.json")
        variant_files.append((scale, solute_npz, out_json))

    # ---------------- 4. Run all_grid_orca_parallel for each scale ----------------
    print("\nRunning all_grid_orca_parallel.py for all scales:\n")
    for scale, npz_file, out_json in tqdm(variant_files, desc="ORCA grid scans", unit="run"):
        print(f"\n*** Scale {scale:.3f}")
        ...
        try:
            ...
            print(f"ERROR: all_grid_orca_parallel.py failed for {npz_file}: {e}",
                  file=sys.stderr)
